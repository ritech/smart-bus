...
module.exports = {
  ...
  0x193A: {
    parse: function(buffer) {
      return {
        status: Boolean(buffer.readUInt8(8)), //1 on. 0 off.
        acnr: buffer.readUInt8(0),
		
        temperature: {
          type: buffer.readUInt8(1),//0 celsius. 1 farenheit
          now: buffer.readInt8(2),
        ...
        modeandfan: buffer.readUInt8(7),
		setupmode: buffer.readUInt8(9),//0 cooling. 1 heating. 2 fan. 3 auto. 4 dry
		setupspeed: buffer.readUInt8(10),
		currentmode: buffer.readUInt8(11),
		sweep: buffer.readUInt8(12)
      };
    },
    ...
    encode: function(data) {
      ...
      buffer.writeUInt8(data.acnr, 0);
	  buffer.writeUInt8(temperature.type, 1);
      buffer.writeInt8(temperature.now, 2);
      buffer.writeUInt8(data.status ? 8 : 0, 2);
      buffer.writeUInt8(data.mode, 9);
